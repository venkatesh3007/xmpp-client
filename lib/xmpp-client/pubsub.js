...
Pubsub.prototype.createNodeWithName = function(node, callback) {
	var jabber = this.client;
...
Pubsub.prototype.publish = function(node, content) {
	...
	attributes.jid = "eassie@roidsoftware.com";

	// var stanza = new builder.Element(
	// 		"iq",
	// 		{to: "pubsub.roidsoftware.com", type: "set", id: uuid.v4()}
	// 	).c('pubsub', {xmlns: 'http://jabber.org/protocol/pubsub'})
	// 	.c('publish', attributes);

	// 	var item = stanza.c('item', {});
	// var xmppElement = item.c('body').t(content).up();
	this.client.publishIqSet(attributes, content, function(iq) {
