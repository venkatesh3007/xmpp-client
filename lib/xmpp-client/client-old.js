...
Client.prototype.iqSet = function(to, query, callback, errorCallback) {
	var n = 'node' + this._iq++;
	this._iqCallback[n] = {};
	...
	if(errorCallback != null) {
		this._iqCallback[n].error = errorCallback;
	}
	...
	var xmppElement = new xmpp.Element('iq', attrs).cnode(query).tree();
	console.log(xmppElement.toString());
	this.xmpp.send(new xmpp.Element('iq', attrs).cnode(query).tree());
	return n;
};

Client.prototype.publishIqSet = function(attributes, content, callback) {
	var n = 'node' + this._iq++;
	if (callback != null) {
		this._iqCallback[n] = {};
		this._iqCallback[n].success = callback;
	...
	var stanza = new builder.Element(
			"iq",
			{to: "pubsub.roidsoftware.com", type: "set", id: n}
		).c('pubsub', {xmlns: 'http://jabber.org/protocol/pubsub'})
		.c('publish', attributes);

	var item = stanza.c('item', {});
	item.c('body').t(content);
	var xmppElement = stanza.up().up();
	this.xmpp.send(xmppElement);
	return n;
}

Client.prototype.affiliationsIqGet = function (node, callback, errorCallback) {
	var n = 'node' + this._iq++;
	this._iqCallback[n] = {};
	if (callback != null) {
		this._iqCallback[n].success = callback;
	...
	var stanza = new builder.Element(
		"iq",
		{to: "pubsub.roidsoftware.com", type: "get", id: n}
	).c('pubsub', {xmlns: 'http://jabber.org/protocol/pubsub'})
	.c('affiliations', {node: node}).up().up();
	console.log(stanza.toString());
	this.xmpp.send(stanza);
	return n;
}

Client.prototype.defaultConfigurationIqGet = function (callback, errorCallback) {
	var n = 'node' + this._iq++;
	this._iqCallback[n] = {};
	if (callback != null) {
		this._iqCallback[n].success = callback;
	...
	.c('default', {}).up().up();
	console.log("Request Stanza: " + stanza.toString());
	this.xmpp.send(stanza);
	return n;
}
